...
def load_module(shortname):
    if shortname.startswith("__"):
        ...
        # support for morphobot originals
        sys.modules["morphobot.utils"] = userbot.utils
        sys.modules["morphobot"] = userbot
...
def admin_cmd(pattern=None, command=None, **args):
    ...
    if pattern is not None:
        if pattern.startswith(r"\#"):
            ...
            args["pattern"] = re.compile(morphoreg + pattern)
    ...
    if pattern is not None:
        if pattern.startswith(r"\#"):
            ...
            if len(Config.SUDO_COMMAND_HAND_LER) == 2:
                morphoreg = "^" + Config.SUDO_COMMAND_HAND_LER
                reg = Config.SUDO_COMMAND_HAND_LER[1]
            elif len(Config.SUDO_COMMAND_HAND_LER) == 1:
                morphoreg = "^\\" + Config.SUDO_COMMAND_HAND_LER
                reg = Config.COMMAND_HAND_LER
            args["pattern"] = re.compile(morphreg + pattern)
            if command is not None:
                cmd = reg + command
            else:
                cmd = (
...
async def unsavegif(event,mafiapikachu):
    try:
        await event.client(
            functions.messages.SaveGifRequest(
                id=types.InputDocument(
                    id=mafiapikachu.media.document.id,
                    access_hash=mafiapikachu.media.document.access_hash,
                    file_reference=mafiapikachu.media.document.file_reference,
                ),
                unsave=True,
            )
        )
